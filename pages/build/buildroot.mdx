...
3. Go into the cloned buildroot directory and run `sudo ./build make` or if you want to, you can first make changes to Busybox(init), Linux and Buildroot settings, once you're done just run `sudo ./build make`.

- `sudo ./build make busybox-menuconfig`
- `sudo ./build make linux-menuconfig`
- `sudo ./build make menuconfig`

4. Once it's done, your ISO file should be in `/images`!
